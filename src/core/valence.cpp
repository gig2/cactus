...
void Valence::valence_init( const MeshT &mesh )
{
    ...
    std::copy( std::begin( valence_par_points ), std::end( valence_par_points ),
               std::back_inserter( valence_par_points_trie ) );
    std::sort( std::begin( valence_par_points_trie ), std::end( valence_par_points_trie ) );
}

float Valence::moyenne_valence() const
{
    int total_valence = 0;
...
float Valence::mediane_valence() const
{
